 ...
 * Usage: node server/scripts/restore-db.js --uri=<target_uri> --dir=<backup_dir> [--drop]
 */
...
async function main() {
  ...
  if (!uri || !backupDir) {
    console.error('Error: --uri=<target_uri> and --dir=<backup_dir> are required.');
    process.exit(1);
  }

  const resolvedDir = path.resolve(process.cwd(), backupDir);
  // Support both direct folder or nested bugboard/ folder
